...
- 仓库包含 327 项本地回归测试，覆盖存档往返、引用完整性、知识边界、行动时序、记忆归属、物品转移和多实体事件等机制。
...
- `state-runtime` 桥接目前是可选的增量集成，只作为通用校验器使用；
  每次校验都从当前 WorldLedger 新建投影，WorldLedger 仍是唯一的状态和事件账本，
  不会自动替换全部旧事件。
- 需要时可在调用 `world_pulse(..., use_state_runtime=True)` 时启用这层校验；
  默认值为 `False`，未安装 `state-runtime` 也不影响原有心跳路径。
