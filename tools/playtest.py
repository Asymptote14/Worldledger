...
局 2：彗星镇（时刻锚）——跑到第七夜，看既定时刻到点必发、地点正确
...
COMET = ("少年与少女每晚在梦中交换身体，两人相隔三年的时光。第七夜，"
         "彗星落下，系守镇被陨石碎片击中：少女与镇民死去，完全停摆——"
         "如雾中之人，不再有心跳、不再行动、与所有场景断联，只有世界"
         "账本记得他们存在；交换自此永断。三年后，少年靠着记忆里残存"
         "的风景、发带与口嚼酒的线索，前往已成废墟的系守镇。黄昏之时"
         "（逢魔时刻）的唤回只有一次：饮下口嚼酒，少女被唤回。")
...
def world2(llm):
    print(SEP + " 局 2 · 彗星镇：既定时刻到点必发" + SEP)
    w = generate_world(llm, "彗星镇", COMET)
    print(f"✨ {w.name} 诞生：{w.law_profile.atmosphere}｜{w.now()}")
